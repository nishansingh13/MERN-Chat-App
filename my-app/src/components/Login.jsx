  ...
  const isIpad = useMediaQuery({query:"(max-width:1500px)"})
   ...
   const {user} =ChatState();
    ...
    const sendotp = async (e) => {
      ...
      try {
        setLoading(true);
        const res = await axios.post("http://192.168.1.11:5000/send-otp", { email });
        setotpstatus(true);
        if (res.status === 200) {
          toast({
    ...
    const hlogin = async (e) => {
      ...
      try {
        ...
        const res = await axios.post("http://192.168.1.11:5000/api/user/login", login_data);
   ...
   const hsignup = async()=>{
    ...
    console.log(pic);
      ...
      try {
        const response = await axios.post("http://192.168.1.11:5000/api/user", data);
